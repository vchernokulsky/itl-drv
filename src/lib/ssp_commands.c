...
SSP_RESPONSE_ENUM ssp_payout_amount(SSP_COMMAND_SETUP setup, const unsigned long amount)
{
	...
	//TODO: change currency type by parameter
    sspC.CommandData[5] = 'R';
    sspC.CommandData[6] = 'U';
    sspC.CommandData[7] = 'B';
...
SSP_RESPONSE_ENUM ssp_set_routing(SSP_COMMAND_SETUP setup,const unsigned long value, const unsigned char route)
{
	...
	//TODO: change currency type by parameter
    sspC.CommandData[6] = 'R';
    sspC.CommandData[7] = 'U';
    sspC.CommandData[8] = 'B';
    resp = _ssp_return_values(setup.port, &sspC);
    return resp;
}
...
SSP_RESPONSE_ENUM ssp_float_payout(SSP_COMMAND_SETUP setup,const unsigned long value,  const unsigned long minimum_payout)
{
    ...
    for (i = 0; i < 4; ++i)
        sspC.CommandData[i+1] = ((minimum_payout >> (8*i))& 0xFF);
    for (i = 0; i < 4; ++i)
        sspC.CommandData[i+5] = ((value >> (8*i))& 0xFF);
    resp = _ssp_return_values(setup.port, &sspC);
    return resp;
}
